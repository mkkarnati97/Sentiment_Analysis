...
model = Sequential()
model.add(Embedding(15001, 300, input_length = train_tweets.shape[1]))
...
history = model.fit(train_tweets, pd.get_dummies(train_data['Sentiment']).values, epochs = 5, batch_size = 128, validation_split = 0.2)
...
my_dict3 = [{'Accuracy': accuracy, 'Precision': precision, 'Recall': recall, 'F_score': fscore, 'AUC': auc, 'Model': 'RNN', 'Textmining_model':'Word_embedding'}]
fields = ['Textmining_model', 'Model', 'Accuracy', 'Precision', 'Recall', 'F_score', 'AUC']
filename = "Cornell_Tweets.csv"
#writing data to csv file

with open(filename, 'a') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames = fields)
    writer.writerows(my_dict3)
